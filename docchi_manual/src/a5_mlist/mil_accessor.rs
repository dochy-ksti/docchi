...
impl MlistMItem {
	...
	pub fn inner_list(&self) -> MListConst<InnerListMItem>{
		let mil = mitem::get_mil_const(self.ptr, "inner_list").unwrap().unwrap();
		MListConst::new(mil, self)
	}
...
#[derive(Debug, PartialEq, Clone, Copy)]
pub struct InnerListMItem {
	ptr : MItemPtr,
}
impl From<MItemPtr> for InnerListMItem {
	fn from(ptr : MItemPtr) -> Self {
		Self{ ptr }
	}
}
