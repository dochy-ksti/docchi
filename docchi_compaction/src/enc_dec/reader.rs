...
impl<'a, T : Read> Reader<'a, T>{
    pub(crate) fn new(read : &'a mut T) -> Self{ Self{ read, buf : Vec::new(), len : 0 } }
    pub(crate) fn read_byte(&mut self) -> ComResult<u8>{
        let mut buf = [0u8; 1];
        self.read.read_exact(&mut buf)?;
        self.len += 1;
        Ok(buf[0])
    }
    pub(crate) fn read(&mut self, bytes : usize) -> ComResult<&[u8]>{
        self.set_buf_len(bytes);
        self.read.read_exact(&mut self.buf)?;
        self.len += self.buf.len();
        Ok(&self.buf)
    }
    pub(crate) fn read_string(&mut self, bytes : usize) -> ComResult<String>{
    ...
    pub(crate) fn read_binary(&mut self, bytes : usize) -> ComResult<Vec<u8>>{
        let mut buf : Vec<u8> = vec_with_capacity_safe(bytes)?;
        unsafe{ buf.set_len(bytes); }
        self.read.read_exact(&mut buf)?;
        self.len += buf.len();
    ...
    pub(crate) fn read_binary8(&mut self, len : usize) -> ComResult<Vec<u64>>{
    ...
    pub(crate) fn read_binary4(&mut self, len : usize) -> ComResult<Vec<u32>>{
    ...
    pub(crate) fn read_binary2(&mut self, len : usize) -> ComResult<Vec<u16>>{
    ...
    pub(crate) fn bytes_read(&self) -> usize{ self.len }
